...
- **What AI generated**: not a single file — a *pattern* across the whole P-005/P-007 session: `client/src/features/landing/`, `client/src/features/auth/components/AuthLayout.jsx`, `client/src/features/auth/pages/ActivateAccountPage.jsx`, and `client/src/features/dashboard/components/ComingSoonPage.jsx` (all built in P-003) were left as untracked, never-`git add`-ed files across every P-005 and early-P-007 commit, while `client/src/App.jsx` (which *was* committed in those same commits) imports directly from `./features/landing/LandingPage.jsx` and the others.
- **What was wrong**: every P-005/P-007 commit up to `033b13b` left the actual GitHub repository in a state that could not build — `App.jsx`'s imports pointed at files that only existed on the local disk, never in git. This was invisible throughout the session because every deploy used `vercel --prod` from the CLI, which uploads the local working directory directly and never touches git at all. The gap surfaced only when Ankush's own `git push` (or Vercel's GitHub-integration webhook reacting to it) triggered an auto-deploy that clones from the actual remote — that clone failed immediately with `Could not resolve "./features/landing/LandingPage.jsx" from "src/App.jsx"`. The original root cause (deciding, in the P-005 session, "these are P-003's pre-existing uncommitted files, not mine to commit") was a reasonable-sounding boundary that stopped being valid the moment any *new* commit started depending on those files — and nothing enforced that dependency check.
- **How I identified it**: the user reported "deployment failed" and "unable to login on localhost"; `vercel ls` showed a `● Error` production deployment from ~5 minutes before their message, and `vercel inspect <url> --logs` showed the exact Rollup resolution error.
- **How I fixed it**: committed all 18 previously-untracked files in one commit (`86b28a9`), then verified with a genuine `git clone` into a scratch directory (not just a local build, which would never have caught this) followed by `npm install && npm run build` — confirmed clean. Re-verified the live site and the now-succeeding GitHub-integration deploy afterward.
- **Lesson → rule added to AGENTS.md?**: Yes — added to `AGENTS.md` §11 (Git): before deciding *not* to commit a pre-existing uncommitted file or directory, check whether anything already committed (or about to be committed) imports from it — if so, it must be committed too, regardless of who originally wrote it. Also: periodically verify the deployable state of the repo via a real `git clone` + build in a scratch directory, not just `vercel --prod` (which uploads the local working tree and can mask exactly this class of gap) or a local `npm run build` (which also reads the local working tree, not git).
